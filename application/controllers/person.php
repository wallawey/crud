...
// 	}
	...
	function index($offset = 0){
        ...
        // generate pagination
        $tot_rec = $this->personModel->count_all();
        ...
        $this->table->set_heading('No', 'Name', 'Phone No', 'Actions');
        $i = 0 + $offset;
        foreach ($persons as $person){
        //,'data-toggle'=>'modal','data-target'=>'#myModal'
            $this->table->add_row(++$i, $person->name,date('d-m-Y',strtotime($person->dob)), 
                anchor('person/view/'.$person->id,'view',array('class'=>'view','data-toggle'=>'modal','data-target'=>'#myModal'.$i)).' '.
                anchor('person/update/'.$person->id,'update',array('class'=>'update','data-toggle'=>'modal','data-target'=>'#myModal'.$i)).' '.
    ...
    function index_dup($offset = 0){
        ...
        $data['table'] = $this->table->generate();
        // load view
        $this->load->view('personList_dup', $data);
	}
	
	/* TESTING PURPOSE
	function hihi(){
	 $data = array();
// 	 $data = $this->personModel->get_paged_list($this->limit, $offset)->result();
     $data[] = array("id" => "fff","title" => "koko");
